...
function EditListing() {
    ...
    if (isMounted) {
      onAuthStateChanged(auth, (user) => {
        if (user) {
          ...
          // userRef is equal to uid of the current user
        } else {
          navigate('/sign-in')
        }
      })
  ...
  const onSubmit = async(e) => {
    ...
    // add location var to formDataCopy object
  ...
  const onMutate = e => {
    ...
    if (!e.target.files) {
      setFormData((prevState) => ({
        ...
        // ?? -> double question mark (if the value on the left is null, then use the right side)
